...
pool_matrix = pd.read_excel('data/sensing_matrix_9drugs.xlsx')
pool_matrix[np.isnan(pool_matrix)] = 0

'''
# construct cmap
flatui = ["white", "lightcoral"]
my_cmap = ListedColormap(sns.color_palette(flatui).as_hex())

plt.figure(dpi = 300, figsize = (7.2,6))
sns.heatmap(pool_matrix, cmap = my_cmap, linewidths=2, cbar=False, linecolor='black')
plt.xticks([])
plt.yticks([])
'''
...
# read protein table
protein_table = pd.read_csv('data/preprocessed_9drugs.csv')

# processing
scores, fold_changes = post_analysis(protein_table, pool_matrix, drug_num = 3)

# save results
scores.to_csv('results/independent_9drugs.csv')
...
for d, genes in drug_targets.items():
    plot_results(d, scores, fold_changes, genes, fc_thres = 1.05, score_thres=0.15, top_markers = 8)
...
drugs = ['Panobinostat', 'Fimepinostat']
genes = ['HDAC1', 'HDAC1']
